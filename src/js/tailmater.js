 ...
 *     15.  Custom JS              *
 ==================================*/
  ...
  const range_slider_func = function () {
    const range = document.querySelectorAll('[data-type="slider"]');
  ...
  const scrollup_func = function () {
    ...
    window.addEventListener(
      "scroll",
      function () {
        const show_back_top = document.querySelector('[role="navtop"]');
        const currentTop =
          document.body.scrollTop || document.documentElement.scrollTop;

        if (show_back_top != null) {
          const min_header = document.querySelector("header").offsetHeight;
          const classTest = show_back_top.classList.contains("is-visible");
  ...
  const circleprogress_func = function () {
    const counts = document.querySelectorAll('[role="progress_bg"]');

    if (counts != null) {
      const circle = document.querySelectorAll('[role="progress_fill"]');

      for (let i = 0; i < counts.length; i++) {
        let val = counts[i].getAttribute("data-percent");

        if (isNaN(val)) {
          val = 100;
        } else {
          const r = circle[i].getAttribute("r");
          const c = Math.PI * (r * 2);
          if (val < 0) {
            val = 0;
          }
